...
int mhi_init_dev_ctxt(struct mhi_controller *mhi_cntrl)
{
	...
	for (i = 0; i < mhi_cntrl->max_chan; i++, chan_ctxt++, mhi_chan++) {
		...
		tmp = chan_ctxt->chcfg;
		tmp &= ~CHAN_CTX_CHSTATE;
		tmp |= MHI_CH_STATE_DISABLED & CHAN_CTX_CHSTATE;
		tmp &= ~CHAN_CTX_BRSTMODE;
		tmp |= mhi_chan->db_cfg.brstmode & CHAN_CTX_BRSTMODE;
		tmp &= ~CHAN_CTX_POLLCFG;
		tmp |= mhi_chan->db_cfg.pollcfg & CHAN_CTX_POLLCFG;
	...
	for (i = 0; i < mhi_cntrl->total_ev_rings; i++, er_ctxt++,
		...
		tmp &= ~EV_CTX_INTMODC;
		tmp &= ~EV_CTX_INTMODT;
		tmp |= mhi_event->intmod & EV_CTX_INTMODT;
